...
def backtest(cl_name,cl_canshu,s_time_s,pkl_tolocal_path):
    import pickle
    import datetime as dt
    from vnpy.event import EventEngine
    from vnpy.app.ib_cta_backtester.engine import BacktesterEngine
    ...
    canshu = {'celue_name': str(cl_name)[:-2], 'huice_s_t': [s_time_s, s_time_s+dt.timedelta(days=27)],
              'setting': {'ma_len':cl_canshu[0],'bd':cl_canshu[1],"dk_len":cl_canshu[2],'acc':cl_canshu[3],'zhouqi':float(str(cl_name).split('_')[-1])}}


    print(canshu)
    ib_backtester.run_backtesting(class_name=canshu['celue_name'], vt_symbol='HSI9999.HKFE', interval='1m',
                                  start=canshu['huice_s_t'][0], end=canshu['huice_s_t'][1],
                                  rate=0.000018, slippage=1, size=100, pricetick=1, capital=1000000,
                                  setting=canshu['setting'], inverse=False)
    strategy_statistics = ib_backtester.get_result_statistics()
    # print(strategy_statistics)
    per_trades = ib_backtester.get_all_trades()
    # list_res = [strategy_statistics['total_net_pnl'], strategy_statistics['max_drawdown'],
    #             strategy_statistics['total_trade_count'], strategy_statistics['sharpe_ratio']]

    if 0 == True:
        per_trades_list = [
            {'datetime': x.datetime.replace(tzinfo=None), 'price': x.price, 'size': x.volume, 'direction': str(x.direction).split('.')[-1].lower(),
             'param1': 0, 'symbol': f'HSI{x.datetime.strftime("%y%m")}'} for x in per_trades]
        print(per_trades_list)
        to_path = pkl_tolocal_path + f'{cl_name}_{cl_canshu}_{s_time_s.strftime("%Y_%m_%d")}' + 'test.pkl'
        with open(to_path, 'wb') as f:
            pickle.dump(per_trades_list, f)
if __name__ == '__main__':
    ...
    if 1==True:
        ...
        try:
            canshu = {'celue_name': 'macd02', 'huice_s_t': [dt.datetime(2020, 6, 1, 0, 0), dt.datetime(2020, 6, 28, 0, 0)],
                      'setting': {"n1":26, "n2":12,'n3':9,'duan':0,'big':0,'only_long':0,'only_short':1}}

            # canshu = {'celue_name': 'ema_tp_04', 'huice_s_t': [dt.datetime(2020, 6, 1, 0, 0), dt.datetime(2020, 6, 28, 0, 0)], 'setting': {'ma_len':80,'bd':2,"dk_len":30,'acc':2,'zhouqi':1}}
            ...
            ib_backtester.run_backtesting(class_name=canshu['celue_name'], vt_symbol='HSI9999.HKFE', interval='1m',
                                          ...
                                          setting=canshu['setting'], inverse=False)
            strategy_statistics = ib_backtester.get_result_statistics()
            ib_backtester.backtesting_engine.show_chart()
            print(strategy_statistics)
