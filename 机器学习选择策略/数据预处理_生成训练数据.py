...
def pack_to_traindatas2(data_path, train_path, zong_t,time_lidu,hc_zq=3):
    ...
    #  2.1读取训练数据地址，保存一个空文件。
    time0 = time.strftime("%m-%d", time.localtime())
    train_path = train_path.split('.')[0] + f'_周期{time_lidu}_回测周期{hc_zq}_{time0}' + '.pickle'
    if os.path.exists(train_path):  # 读取保存地址
        print(f'地址存在{train_path}。')
...
if __name__ == '__main__':
    ...
    # zong_t = [dt.datetime(2019,6, 1), dt.datetime(2020, 10, 1)]
    ...
    if 1 == True:
        ...
        for sn in ['01','02','03','04']:
            data_path = r'F:\new_0811\DB_in_out_strategies\res_df_in_dict_to%s_11-11_allnew.pickle'%str(sn)
            # 一次训练数据
            train_path = r'F:\new_0811\huice_log\train_data\MEBD_%s_每月训练数据.pkl'%str(sn)
            time_lidu = 1
            zong_t = [dt.datetime(2019, 6, 1), dt.datetime(2020, 10, 1)]
            pack_to_traindatas2(data_path, train_path, zong_t,time_lidu=1,hc_zq=3)
