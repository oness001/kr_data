...
def poly_ridge_regression(X_train=[], yinzi=[], y_train=[], degree=1):
    from sklearn.model_selection import cross_validate
    from sklearn.preprocessing import PolynomialFeatures
...
def randomforest_classify_grid(X_train=pd.DataFrame(),yinzi=[],y_train=pd.DataFrame(),n_iter=5,n_jobs=2):
    ...
    if X_train.shape[0]< cv_n:
        cv_n = X_train.shape[0]  if X_train.shape[0] >0 else 1
        n_iter = X_train.shape[0]  if X_train.shape[0] >0 else 1
    grid = RandomizedSearchCV(RandomForestClassifier(), param_grid,n_iter=n_iter, cv=cv_n,n_jobs=n_jobs)
