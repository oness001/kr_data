...
def corr_jw(train_data=None,n=10, yinzi=[],res_index = '预测周期真实收益'):
    ...
    for i in yinzi:
        # print(i)
        # print(min(np.corrcoef(train_data[i], train_data)[0]))
...
def zuhe_911_022(train_data=None,res_to_df ={} ,yinzi=[], Train=True, last=False, n_jobs=2,**canshu):
    '''
    数据归一化
    第一次分类，adaboost算法
    第二次分类，random——Forrest分类
    ...
    最后进行回归。
    '''
    if 'data' not in (res_to_df.keys()):
        res_to_df['data'] =pd.DataFrame()
    ...
    if Train:
        ...
        if last == False:
            ...
            if train_data0[con1]['预测周期真实收益'].empty:
                print('kongzhi!')
            else:
                # print(train_data0[con1]['预测周期真实收益'].tail(100))
                print(train_data0[con1]['预测周期真实收益'].shape)
                train_data0.loc[con1, '预测周期真实分类'] = pd.qcut(train_data0[con1]['预测周期真实收益'], q=int(q / 2),
                                                        labels=[x for x in range(1, int(q / 2) + 1, 1)], precision=1)
            try:
                if train_data0[con2]['预测周期真实收益'].empty:
                    print('kongzhi!')
                else:
                    # print(train_data0[con2]['预测周期真实收益'].tail(100))
                    print(train_data0[con2]['预测周期真实收益'].shape)
                    train_data0.loc[con2, '预测周期真实分类'] = pd.qcut(train_data0[con2]['预测周期真实收益'], q=int(q / 2),
                                                                labels=[x for x in range(int(q / 2) + 1, q + 1, 1)], precision=1)
            except :
                train_data0.loc[con2, '预测周期真实分类'] = pd.qcut(train_data0[con2]['预测周期真实收益'], q=int(q / 4),
                                                            labels=[x for x in range(int(q / 2) + 1, q + 1, 2)], precision=1)
            # exit()
        else:
            train_data0['预测周期真实分类'] = 0
    # # 数据处理：==》False
    ...
    for ix,i in enumerate(fl_list):
        ...
        if Train and last == False:
            ...
            train_data0[f'{ix}训练集_best_score'] = scores[0]
            train_data0[f'{ix}训练集_sj_acc_score'] = scores[1]
            train_data0[f'{ix}训练集_sj_ba_acc_score'] = scores[2]
            train_data0[f'{ix}model_name'] = model_name
            train_data0[f'{ix}训练集_预测值'] = X_train_pre['预测值']
            train_data0 = train_data0[['canshu','celue_name', f'{ix}model_name','训练集_真实收益','训练集_真实分类',
                                                     f'{ix}训练集_best_score', f'{ix}训练集_sj_acc_score', f'{ix}训练集_sj_ba_acc_score',f'{ix}训练集_预测值']]
            res_to_df[model_name] = model1
            data_t = res_to_df[pre_res_name]
            if data_t.empty:
                res_to_df[pre_res_name] = train_data0
                continue
            res_to_df[pre_res_name] = pd.merge(data_t, train_data0, on=[ 'celue_name', 'canshu'], how='inner',suffixes=('','_重复'))
        # 预测》》预测结果
        elif Train==False and last == False:
            print(f'预测模型：{model_name}数据量：{train_data0.shape}，因子量:{len(yinzi)}')
            # 拿到预测模型
            model1 = res_to_df[model_name]
            cat_pre = model1.predict(train_data0[yinzi])
            train_data0[f'{ix}预测集_预测值'] = pd.Series(cat_pre, index=train_data0.index)
            train_data0.loc[:, pre_res_name] = pd.Series(cat_pre, index=train_data0.index)
            train_data0[f'{ix}model_name'] = model_name
            train_data0 = train_data0[[ 'canshu', 'celue_name',  f'{ix}model_name','预测周期真实收益','预测周期真实分类',f'{ix}预测集_预测值']]
            data_t = res_to_df[pre_res_name]
            res_to_df[pre_res_name] = pd.merge(data_t, train_data0, on=[ 'celue_name', 'canshu'], how='inner',suffixes=('','_重复'))
            # print(train_data0.keys())
...
def second_fl01(train_data=None,res_to_df ={} ,yinzi=[],res_index=[], Train=True,  n_jobs=2,**canshu):
    '''
    数据归一化
    第一次分类，adaboost算法
    第二次分类，random——Forrest分类
    ...
    def data_fliter_fb(data):
        # 过滤算法
        # data = data[data['total_days_3'] > 4]
        data = data[data['训练_trade_nums_3'] > 10]
        # data = data[data['trade_nums_2'] > 10]
        data = data[data['训练_trade_nums_3'] < 150]
        # data = data[data['trade_nums_2'] < 200]
        # data.fillna(0, inplace=True)
        pass
    ...
    # pac_yinzi
    yinzi_k = [x for x in yinzi if not str(x).startswith('clm')]
    # 剩余因子
    yinzi_y = [x for x in yinzi if x not in yinzi_k]

    # print('pac计算因子特征：',len(yinzi_k))
    # print('参数和策略：',len(yinzi_y))
    train_data1 = data_precess(train_data,yinzi_k,'standar')
    for col in train_data1.keys():
        train_data[col] = train_data1[col]

    train_data0=train_data
    print(train_data0.tail(5))
    # exit()
    ...
    if Train:
        ...
        # print(yinzi_k)
        # exit()
        # con1 = train_data0[resinx] <= 0
        # con2 = train_data0[resinx] > 0
        # train_data0.loc[con1, '预测周期真实分类2'] = pd.qcut(train_data0[con1][resinx], q=int(q / 2),
        #                                             labels=[x for x in range(1, int(q / 2) + 1, 1)], precision=1)
        # train_data0.loc[con2, '预测周期真实分类2'] = pd.qcut(train_data0[con2][resinx], q=int(q / 2),
        #                                             labels=[x for x in range(int(q / 2) + 1, q + 1, 1)], precision=1)
        train_data0.loc[:, '预测周期真实分类2'] = pd.qcut(train_data[resinx], q=int(q ),
                                                 labels=[x for x in range(1, int(q) + 1, 1)], precision=1)
        train_data0.dropna(axis=0, subset=['预测周期真实分类2'], inplace=True)
    ...
    for ix , i in enumerate(fl_list):
        ...
        if Train:
            ...
            train_data0.rename(columns={'预测周期真实分类2' : '二次训练_真实分类'},inplace =True)
            train_data0.rename(columns={resinx:'二次训练_真实收益'},inplace =True)
            train_data0.rename(columns={'s_Time':'二次训练_s_Time'},inplace =True)

            train_data0[f'二次训练_预测值{ix+1}'] = X_train_pre['预测值']
            train_data0[f'二次训练_best_score{ix+1}'] = scores[0]
            train_data0[f'二次训练_sj_acc_score{ix+1}'] = scores[1]
            train_data0[f'二次训练_sj_ba_acc_score{ix+1}'] = scores[2]
            train_data0[f'二次训练_model_name{ix+1}'] = model_name

            res_to_df[pre_res_name] = train_data0[['canshu','celue_name','二次训练_s_Time','二次训练_真实分类','二次训练_真实收益',f'二次训练_预测值{ix+1}',f'二次训练_best_score{ix+1}',f'二次训练_sj_acc_score{ix+1}',f'二次训练_sj_ba_acc_score{ix+1}']+yinzi]
            print('分类训练：\n',res_to_df[pre_res_name].tail(2))
        ...
        # 预测》》预测结果
            ...
            data_t = res_to_df[pre_res_name]
            model1 = res_to_df[model_name]
            train_data00 = train_data0[yinzi]
            train_data00.fillna(0,inplace =True)
            cat_pre = model1.predict(train_data00)

            train_data0.rename(columns={'s_Time':'预测_s_Time'},inplace =True)
            train_data0[f'最终预测值{ix+1}'] = pd.Series(cat_pre, index=train_data0.index)
            train_data0[f'预测_model_name{ix+1}'] = model_name

            train_data0 = train_data0[['canshu','celue_name','预测_s_Time',f'最终预测值{ix+1}',f'预测_model_name{ix+1}']+yinzi+res_index]
            res_to_df[pre_res_name1] = (train_data0)

            # res_to_df[pre_res_name] = (data_t).append(train_data0)
            # res_to_df[pre_res_name] = pd.merge(data_t, train_data0, on=['celue_name', 'canshu'], how = 'outer',suffixes=('','_yc'))

            print('分类预测：\n',res_to_df[pre_res_name1].tail())
...
def second_hg01(res_to_df ={} ,yinzi=[],resinx='', Train=True, **canshu):
    '''
    数据归一化
    第一次分类，adaboost算法
    第二次分类，random——Forrest分类
    ...
    def data_fliter_fb(data,index_hgtj,q):

        # 过滤算法
        # data = data[data['total_days_3'] > 4]
        # print('回归:',data.tail(2))
        data = data[data[index_hgtj] > q/2]
        # data = data[data['trade_nums_2'] > 10]
        # data = data[data['训练_trade_nums_3'] < 130]
        # data = data[data['trade_nums_2'] < 200]
        # data.fillna(0, inplace=True)
        pass
        ...
        return pd.DataFrame(data)
    if Train:
        if 'data' in res_to_df.keys():
            train_data = res_to_df['data']
        else:
            res_to_df['data']=pd.DataFrame()
            return res_to_df
    else:
        if 'yc_data' in res_to_df.keys():
            train_data = res_to_df['yc_data']
        else:
            res_to_df['yc_data'] =pd.DataFrame()
            return res_to_df['yc_data']
    ...
    if resinx not in train_data.keys():
        print('无目标结果数据')
        return res_to_df
    print(f'\n======\n欢迎使用{sys._getframe().f_code.co_name}')
    print(f'本次传入数据量：{train_data.shape}')
    #  ===过滤
    if Train:
        train_data = data_fliter_fb(train_data,index_hgtj=f'二次训练_预测值1',q=q)
    else:
        train_data = data_fliter_fb(train_data,index_hgtj=f'最终预测值1',q=q)
    ...
    #   ==数据过滤
    if Train:
        print(f'本次回归训练，本次数据量：{train_data0[yinzi].shape}')
        # print(train_data.tail(100))
        # train_data0, train_data_, yinzi_k = k_pac(train_data, yinzi_k, N=40, kernel='linear', n_jobs=1)
    else:
        print(f'本次回归预测，本次数据量：{train_data0[yinzi].shape}')
        # train_data0, train_data_, yinzi_k = k_pac(train_data, yinzi_k, N=pac_k, kernel='linear', n_jobs=1)

    yinzi = yinzi_k + yinzi_y
    if train_data0.empty:return res_to_df
    for ix , i in enumerate(fl_list):
        pre_res_name = 'data'
        pre_res_name1 = 'yc_data'
        model_name = 'model_%s'%i
        #训练 》》model
        if Train:
            # print(train_data0.tail(100),'\n\n')
            # print('\n\n',train_data0.shape)
            if train_data0.shape[0]>5000:train_data0 = train_data0.sample(4000)
            else:pass
            print(f'hg训练：{model_name}数据量：{train_data0[yinzi].shape},res_数据量:{train_data0[resinx].shape}，因子量:{len(yinzi)}')
            # print(train_data0['预测周期真实分类'].sample(100))
            model1, X_train_pre, scores = eval(i)(X_train=train_data0, yinzi=yinzi, y_train=train_data0[resinx],degree=3)

            res_to_df[model_name] = model1


            train_data0[f'hg训练_预测值{ix+1}'] = X_train_pre['预测值']
            train_data0[f'hg训练_mean_score{ix+1}'] = scores
            train_data0[f'hg训练_model_name{ix+1}'] = model_name
            data_y  = res_to_df[pre_res_name]
            res_to_df[pre_res_name] = pd.merge(data_y, train_data0[['celue_name', 'canshu','二次训练_s_Time', f'hg训练_mean_score{ix+1}', f'hg训练_预测值{ix+1}']], on=['二次训练_s_Time','celue_name', 'canshu'], how='outer',suffixes=('','_hg'))
            # print(train_data0[resinx].tail())
            # print(data_y[resinx].tail())
            print('hg训练：\n',res_to_df[pre_res_name].tail(2))

        # 预测》》预测结果
        elif Train==False :
            print(f'hg预测=：{model_name}数据量：{train_data0.shape}，因子量:{len(yinzi)}')
            # 拿到预测模型
            data_t = res_to_df[pre_res_name]
            data_yc  = res_to_df[pre_res_name1]
            model1 = res_to_df[model_name]
            cat_pre = model1.predict(train_data0[yinzi])

            train_data0[f'hg最终预测值{ix+1}'] = pd.Series(cat_pre, index=train_data0.index)
            train_data0[f'hg训练_model_name{ix+1}'] = model_name
            train_data0=train_data0[[f'hg训练_model_name{ix + 1}', 'celue_name', 'canshu', '预测_s_Time', resinx, f'hg最终预测值{ix + 1}']]
            # res_to_df[pre_res_name] = pd.merge(data_t, train_data0, on=['预测_s_Time','celue_name', 'canshu'], how='outer',suffixes=('','_hg'))
            res_to_df[pre_res_name1] = pd.merge(data_yc,train_data0, on=['预测_s_Time','celue_name', 'canshu'], how='outer',suffixes=('','_hg'))
            print('hg预测：\n ',res_to_df[pre_res_name1][[x for x in list(res_to_df[pre_res_name1].keys()) if x not in yinzi]].tail())
            # print(data_t[resinx].tail())
            # print(res_to_df[pre_res_name].tail())
    return res_to_df
