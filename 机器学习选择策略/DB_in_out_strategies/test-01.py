...
from mongoengine import register_connection, Document, DateTimeField, StringField, BinaryField, FloatField, BooleanField, DictField
...
class StrategiesSet(Document):
    ...
    meta = {
        ...
        "indexes": [
            {
                "name": "ss_unique",
                "fields": ("class_name", "vt_symbol", "interval", "start", "end", "rate",
                           "slippage", "size", "pricetick", "capital", "inverse", "setting"),
                "unique": True,
            },
            {
                "fields": ("class_name",),
...
if __name__ == '__main__':
    ...
    init()
    logger.info(f'使用{args.nCPU}核心回测策略：{args.strategies}')
    st = dt.datetime.now()
