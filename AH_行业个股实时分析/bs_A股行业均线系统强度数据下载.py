...
def get_a_stock(code_name='sh.600004',s_time ="2020-10-23",e_time = "2020-10-31", frequency = 'd'):
    ...
    return result
# 获取单个股票的日线数据,默认前复权。
async def get_a_stock_kline(code_name='sh.600000',s_time ='2010-07-01',e_time = '2017-12-31', frequency = 'd'):
    ...
    df = MA_Sys(df=result)
    df = zdf_cal(df=df)
    df_to_save = df.iloc[-30:].copy()
    # print(df_to_save)
    print("time:",(time.time_ns()-x0)/1000000000,"s")
    return df_to_save

async def main(stock_list,s_time ,e_time ):
    # loop = asyncio.get_event_loop()
    # loop.run_until_complete(asyncio.wait(*tasks))
    tasks = []
    for s0 in stock_list:
        task_1 = asyncio.create_task(get_a_stock_kline(code_name = s0,s_time =s_time,e_time = e_time)) # 2.任务123被创建，并进入事件循环等待运行
        tasks.append(task_1)
