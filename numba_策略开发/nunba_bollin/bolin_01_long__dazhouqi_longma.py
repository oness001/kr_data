...
if __name__ == '__main__':
    ...
    if True == a:
        ...
        df_time_list = [['2017-1-01 09:15:00', '2017-12-26 16:25:00']]
    ...
    if True == b:
        ...
        canshu_list = []
        for cs1 in range(210, 226, 5):
            for cs2 in range(11, 16, 2):
                for cs3 in [i for i in range(130, 191, 10)]:
                    for cs4 in [i for i in range(2000, 3001, 500)]:
                        for cs5 in range(35, 46,5):
                            for cs6 in range(8, 15, 2):
                                canshu_list.append([cs1, cs2, cs3,cs4, cs5/10,cs6])

        print('参数列表个数：', len(canshu_list))
        canshu_list = canshu_list[:]
        to_csv_path = r'csv_gather\%s_%s_总体细.csv' % (filename,timeer0)
