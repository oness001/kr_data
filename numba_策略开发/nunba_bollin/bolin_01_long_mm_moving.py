import os
import sys
sys.path.append(r'F:\vnpy_my_gitee\new_company')
...
def cal_signal_(df0, df1, df2, strat_time, end_time, cs0, sxf=1, slip=1):
    ...
    for i in range(10, df0.shape[0]):  # 类似于vnpy:on_bar
        ...
        if trading_time_con:
            ...
            if we_trade_con:
                if now_pos == 0:
                    long_condition =  (clsoe_bar > up_1) and (last_close <= up_1) and (ma_xiao>da_ma)
...
def cal_signal(df, strat_time, end_time, canshu):
    ...
    df2cols = ['candle_begin_time', '止损价', '止盈价', '日内最低价', '日内最高价', '开仓线',
               '小均线', 'n_atr','移动止赢','大均线','max_n','up','dn']
    # 配置参数:止盈，ma_len（atr），atr_倍数，交易次数
    # stop_win_n, ma_len, da_ma_n,sw_atrn,trade_inner_nums = cs0
    ...
    df['up'] = talib.MIN(df['大均线'] + std_da*canshu[3],canshu[1])
    df['up'] =df['up'].rolling(int(canshu[1])).mean()
...
if __name__ == '__main__':
    ...
    c = 10
    if True == a:
        df_time_list = [['2016-1-01 09:15:00', '2019-12-20 16:25:00']]
        ...
        resdf, res0 = duojincheng_backtesting(df000, zong_can=[[220.0, 13.0, 15.0, 4.0, 10.0]],
    ...
    # [190, 20, 2, 2]
    if True == b:
        # 260.0, 19.0, 2000.0, 5.0, 1.0
        canshu_list = []
        for cs1 in range(180, 240, 5):
            for cs2 in range(13, 22, 1):
                for cs3 in [i for i in range(100, 600, 20)]:
                    for cs4 in range(15, 46, 5):
                        # for cs5 in range(1, 3, 1):
                        canshu_list.append([cs1, cs2, cs3, cs4/10,10])
