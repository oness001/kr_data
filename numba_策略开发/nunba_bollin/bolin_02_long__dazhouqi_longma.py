...
if __name__ == '__main__':
    ...
    if True == a:
        ...
        df_time_list = [['2016-1-01 09:15:00', '2019-12-26 16:25:00']]
        s_time, e_time = df_time_list[0]
        df = get_loc_hsicsv(s_time, e_time, datapath)  # 获取本地数据
        df000 = df.copy()
        time0 = time.process_time()
        df000 = df.copy()
        # [215.000 11.000 185.000 2000.000  4.000  7.000]
        resdf, res0 = duojincheng_backtesting(df000, zong_can=[[215.0, 11.0, 185.0,2000, 4.0, 7.0]],
        ...
        if mode == 3:
            ...
            # , '资金曲线', '资金曲线2'
            resdf['close0'] = resdf['close'] - resdf['close'].shift()
            resdf['close'] = resdf['close0'].cumsum()
    ...
    # [190, 20, 2, 2]
    if True == b:
        # [210.000 11.000 190.000 2000.000  4.000  8.000]
        canshu_list = []
        for cs1 in range(200, 216, 5):
            for cs2 in range(11, 16, 2):
                for cs3 in [i for i in range(180,201,5)]:
                    for cs4 in [i for i in range(1500, 2001, 500)]:
                        for cs5 in range(35,45,5):
                            for cs6 in range(6, 10, 1):
                                canshu_list.append([cs1, cs2, cs3,cs4, cs5/10,cs6])

        print('参数列表个数：', len(canshu_list))
        canshu_list = canshu_list[:]
        to_csv_path = r'csv_gather\%s_%s_细回测.csv' % (filename,timeer0)
        df_time_list = [['2016-1-10 09:16:00', '2019-12-20 16:25:00']]
        s_time, e_time = df_time_list[0]
        df = get_loc_hsicsv(s_time, e_time, datapath)  # 获取本地数据
        df000 = df.copy()
