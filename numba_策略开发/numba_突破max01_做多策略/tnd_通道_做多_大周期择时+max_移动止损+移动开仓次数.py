...
from numba_策略开发.画图工具.echart_可加参数 import draw_charts,only_line,draw_charts2
    ...
    for i in range(10, df0.shape[0]):  # 类似于vnpy:on_bar
        ...
        if trading_time_con:
            ...
            if ((df0[i, 7] == 9) and (df0[i, 8] == 15)):
                if da_ma > df2[i - 100][9]:
                    inner_trade_nums = int(trade_inner_nums+1)
                else:
                    inner_trade_nums = int(trade_inner_nums)
            ...
            if max_log_high > now_open_prcie + stop_win_n:
                ...
                moving_swin_price = max(ma_xiao - swin_atr_n,now_open_prcie- swin_atr_n)
            max_stop_win = max(max_stop_win,moving_swin_price)
            stop_win_con = (clsoe_bar < max_stop_win)

            # 信号生成
            if we_trade_con:
                if now_pos == 0:
                    max_stop_win = min_log_low
                    long_condition =  (clsoe_bar > max_n_p) and (last_close <= max_n_p) and (clsoe_bar>da_ma)
                    if long_condition and inner_trade_nums > 0:
                        inner_trade_nums -= 1
                        df1[i][1] = 1
                elif now_pos > 0:
                    stop_loss_con = False #(clsoe_bar > stop_loss_price) and (last_close <= stop_loss_price)
...
if __name__ == '__main__':
    ...
    if True == a:
        infos = 'test'
        tohtml_path = r'html_gather\%s_%s' % (filename,infos )
        ...
        resdf, res0 = duojincheng_backtesting(df000, zong_can=[[200.0, 19.0, 2000.0, 4.0, 2.0]],
        ...
        if mode == 1:
            ...
            draw_charts2(resdf, canshu={'opne_price': resdf['opne_price'],
                                       '止损价': resdf['止损价'],
                                       '止盈价': resdf['止盈价'],
                                       '日内最低价': resdf['日内最低价'],
                                       '日内最高价': resdf['日内最高价'],
                                       ...
                                       }, res_df=res0,vol_bar=True, markline_show2=True,
                                path=tohtml_path)
        if mode == 2:
    ...
    # [190, 20, 2, 2]
    if True == b:
        # [200.0, 19.0, 2000.0, 4.0, 2.0]
        canshu_list = []
        for cs1 in range(190, 235, 5):
            for cs2 in range(13, 27, 1):
                for cs3 in range(2000, 3001, 100):
                    for cs4 in range(15, 60, 5):
                        for cs5 in range(1, 4, 1):
                            canshu_list.append([cs1, cs2, cs3, cs4/10,cs5])
        canshu_list = canshu_list[:]
        to_csv_path = r'csv_gather\%s_%s_test.csv' % (filename,timeer0)
