...
def cal_signal_(df0, df1, df2, strat_time, end_time, cs0):
    ...
    stop_win_n, ma_xiao_n,ma_zhong_n,ma_da_n,dazhouqi,cr_num, atr_sw_n = cs0
    # 配置临时变量
    ...
    for i in range(10, df0.shape[0]):  # 类似于vnpy:on_bar
        ...
        if trading_time_con:
            ...
            if max_log_high > (now_open_prcie + stop_win_n):
                ...
                stop_win_con = (close_bar < xiao_ma)
            elif max_log_high > (now_open_prcie + stop_win_n):
                stop_loss_price = 0.5*(xiao_ma+zhong_ma)
                stop_win_con = (close_bar < 0.5*(xiao_ma+zhong_ma))
            else:
                stop_loss_price = (now_open_prcie - atr_n)
                stop_win_con = (close_bar < stop_loss_price )

            moving_swin_price =  np.nan
            if we_trade_con:
                if now_pos == 0:
                    long_condition = (xiao_ma > zhong_ma > da_ma)
                    long_condition &= (close_bar>xiao_ma)&(last_close< xiao_ma)
                    long_condition &= (chanraoindex < cr_num)
                    long_condition &= (close_bar > day_ma)
...
if __name__ == '__main__':
    ...
    if True == a:
        ...
        df = transfer_to_period_data(df,rule_type='5T')
        df000 = df.copy()
        ...
        resdf, res0 = duojincheng_backtesting(df000,
                                              zong_can=[[150,18,60,100,3500,100,1.8]],
    ...
    if True == b:
        ...
        canshu_list = []
        for cs1 in range(130, 180, 10):
            for cs2 in range(15, 21, 2):
                for cs3 in range(1500, 4020, 500):
                    for cs4 in range(15, 60, 10):
                        for cs5 in range(3, 9, 2):
