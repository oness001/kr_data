        ...
        if trading_time_con:
            ...
            if we_trade_con:
                if now_pos == 0:
                    ...
                    short_condition &= da_ma > 0
                    short_condition &= da_mastd >0
...
def cal_signal(df, strat_time, end_time, canshu):
    ...
    df2cols = ['candle_begin_time', '止损价', '止盈价', '日内最低价', '日内最高价', '开仓线',
               'atr_day', '小均线', 'n_atr','移动止赢','std_大均线','大均线']
    # 配置参数:止盈，ma_len（atr），atr_倍数，交易次数
    # stop_win_n, ma_len,big_ma, base_line,extend_n = cs0
    df['小均线'] = talib.SMA(df['close'], int(canshu[1]))
    std = talib.STDDEV(df['close'], int(canshu[2]))
    std_ma = talib.SMA(std, int(canshu[2]))
    bigma = talib.SMA(df['close'], int(canshu[2]))
    df['大均线'] =   bigma-bigma.shift(int(canshu[1]))

    df['std_大均线'] = 0
    df.loc[(std_ma > std),'std_大均线'] = 1
    # df['std_大均线'].fillna(0,inplace=True)
    # print(df[df['std_大均线']==False].tail(500))
    # exit()
...
if __name__ == '__main__':
    ...
    if True == a:
        info = '插针反弹_大周期_精细粒度'
        df_time_list = [['2016-1-10 09:20:00', '2019-12-28 16:20:00']]
        ...
        only_line(resdf, zhibiaos=[],canshu=['std_ma','std','资金曲线','资金曲线2'],rule_type='2H', path='资金曲线test.html')
        ...
        if mode == 1:
            ...
            draw_charts(resdf, canshu={'opne_price': resdf['opne_price'],
                                ...
                                path=r'html_gather\%s_%s_%s' %(filename, timeer0, info))
        if mode == 2:
    ...
    # [190, 20, 2, 2]
    if True == b:
        info = '插针反弹_std过滤_精细粒度'

        canshu_list = []

        for cs1 in range(150, 161, 3):
            for cs2 in range(18, 27, 3):
                for cs3 in range(1800, 2501, 100):
                    for cs4 in range(12, 17, 1):
                        for cs5 in range(4, 8, 1):
                            canshu_list.append([cs1, cs2, cs3,cs4, cs5/10])
        canshu_list = canshu_list[:]
        df_time_list = [['2019-1-10 09:15:00', '2019-12-20 16:25:00']]
        s_time, e_time = df_time_list[0]
        df = get_loc_hsicsv(s_time, e_time, datapath)  # 获取本地数据
        df000 = df.copy()
    ...
    if True == c:
        ...
        for cs1 in range(130, 190, 10):
            for cs2 in range(15, 25, 3):
                for cs3 in range(1500, 3501, 500):
                    for cs4 in range(14, 20, 2):
                        for cs5 in range(5, 9, 1):
                            canshu_list.append([cs1, cs2, cs3, cs4, cs5 / 10])
        ...
        strat_time = np.array([1, 9, 30])
        end_time = np.array([27, 16, 10])
        name = f'{filename}_{s_time[:4]}_{e_time[:4]}_16-19_总体回测'
