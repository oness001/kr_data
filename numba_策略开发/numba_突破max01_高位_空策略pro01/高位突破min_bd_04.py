...
def cal_signal_(df0,df1,df2,strat_time,end_time,cs1=0,cs2=0,cs3=0,cs4=0, sxf=1,slip=1):
    ...
    # df1 == 信号统计数据列:np.array ：['candle_begin_time','signal', 'pos', '持仓价', 'per_lr', 'sl']
    # df2 == 指标列:np.array
    '''
    # 配置参数
    stop_loss_n = cs1
    ...
    for i in range(10,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            if we_trade_con:
                ...
                if now_pos == 0:
                    short_condition = (clsoe_bar < low_inner) and (last_close >= low_inner)
                    # 空：突破最低线。
...
def duojincheng_backtesting(df_input,zong_can,strat_time,end_time, cpu_nums=3):
    ...
    df1cols = ['candle_begin_time', 'signal', 'pos','持仓价', 'per_lr', 'sl']
    df1 = df[df1cols].values
    df2cols = ['candle_begin_time', 'open', 'high', 'low', 'close', 'volume',
              '止损价','止盈价','日内最低价','日内最高价','atr_day']
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        df000 = df.copy()
        resdf,res0 = duojincheng_backtesting(df000, zong_can=[[45,250,250,50]],
                                             strat_time=strat_time, end_time=end_time, cpu_nums = 1)

        print(res0)
        resdf['atr_day'] = resdf['日内最低价'] + resdf['atr_day']
        draw_charts(resdf,canshu={'持仓价':resdf['持仓价'],
                                  '止损价' :resdf['止损价'],
                                  '止盈价':resdf['止盈价'],
                                  '日内最低价' :resdf['日内最低价'],
                                  '日内最高价': resdf['日内最高价'],
                        ...
                        },vol_bar=True, markline_show2=True,path='tp_max_04_%s'%timeer0)
        print(f'总时间：{time.process_time() - time0}  s')
    #多进程策略 :
    elif 1:
        canshu_list = []
        for cs1 in range(30, 80, 5):
            for cs2 in range(150, 310, 5):
                # for cs3 in range(200, 320, 10):
                        # for cs4 in range(30, 35, 5):
                canshu_list.append([cs1, cs2, 0,0])
        ...
        print(resdf.tail())
        resdf.to_csv(s_time[:7]+r'_numba_突破max01_策略04_%s.csv'%timeer0)
        print(f'总时间：{time.process_time() -time0}  s')