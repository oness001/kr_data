...
def cal_signal_(df0,df1,df2,strat_time,end_time,cs0, sxf=1,slip=1):
    ...
    '''
    # 配置参数
    stop_loss_n ,stop_win_n,max_bd ,ma_len,tradi_inner_nums = cs0
    # 配置临时变量
    qiangao = np.nan
    inner_trade_nums = tradi_inner_nums
    open_pos_size = 1
    max_log_high = np.nan
    min_log_low = np.nan
    for i in range(10,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            stop_loss_price = qiangao if (now_open_prcie + stop_loss_n)>qiangao else (now_open_prcie + stop_loss_n)
            stop_win_price = now_open_prcie - stop_win_n
            max_bd_line = min_log_low + max_bd
            # 信号生成
            if we_trade_con:
                if now_pos == 0:
                    qiangao = np.nan
                    short_condition = (clsoe_bar > max_bd_line) and (clsoe_bar < ma_xiao) and (last_close >= ma_xiao)
                    # 空：突破最低线。
                    if short_condition and inner_trade_nums > 0:
                        inner_trade_nums -= 1
                        df1[i][1] = -1
                        qiangao = max_log_high
                elif now_pos < 0:
...
def cal_signal(df,strat_time,end_time,canshu):
    ...
    df2cols = ['candle_begin_time', '止损价', '止盈价', '日内最低价', '日内最高价', '开仓线',
               'atr_day', '小均线','n_atr','开仓高点']
    df['小均线'] = talib.SMA(df['close'], int(canshu[3]))
    df['n_atr'] = talib.ATR(df['close'],df['high'],df['low'], int(canshu[3]))
    df['开仓高点'] = np.nan
    # print(df.tail())
    # exit()
...
if __name__ == '__main__':
    ...
    # print(timeer0)
    # exit()
    df_time_list = [['2015-1-01 09:15:00', '2019-12-27 16:25:00']]
    ...
    if True == a:
        ...
        resdf,res0 = duojincheng_backtesting(df000, zong_can=[[50,130,270,20,2]], strat_time=strat_time, end_time=end_time, cpu_nums = 1)
        print(res0)
        print(resdf.tail(5))
        ...
        if mode == 1:
            ...
            draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                        ...
                        },vol_bar=True, markline_show2=True,path=r'html_gather\max_突破06前高_test%s'%timeer0)
        if mode == 2:
            draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                                  '止损价' :resdf['止损价'],
                                  '止盈价':resdf['止盈价'],
                                  ...
                                  '开仓高点': resdf['开仓高点'],
                                  "atr_day":resdf['atr_day']
                        },vol_bar=True, markline_show2=True,path=r'html_gather\max_突破06_前高_%s'%timeer0)
        print(f'总时间：{time.process_time() - time0}  s')
    #多进程策略 :
    if True == b:
        [50, 130, 270, 20, 2]
        canshu_list = []
        for cs1 in range(40, 61, 10):
            for cs2 in range(100, 241, 10):
                for cs3 in range(130, 321, 10):
                        for cs4 in range(10, 34, 3):
                            for cs5 in range(1, 4, 1):
        ...
        print(resdf.tail())
        resdf.to_csv(r'csv_gather\max_突破06_前高_%s.csv'%timeer0)
        print(f'总时间：{time.process_time() -time0}  s')
    ...
    if True == c:
        ...
        name = 'max_突破06_前高_2015_2019'

        if True == 1:
            [50, 130, 270, 20, 2]
            canshu_list = []
            for cs1 in range(40, 61, 10):
                for cs2 in range(110, 180, 10):
                    for cs3 in range(260, 300, 10):
                        for cs4 in range(10, 20, 2):
                            for cs5 in range(1, 3, 1):
                                canshu_list.append([cs1, cs2, cs3, cs4, cs5])

            print('参数列表个数：', len(canshu_list))
            time0 = time.process_time()
            func_canshu = {'zong_can': canshu_list[:], 'cpu_nums': 3, "strat_time": strat_time, "end_time": end_time}
            df_zong = duojieduan_huice(df, duojincheng_backtesting, func_canshu, s_time, e_time, jiange='12MS')
            path_csv = r'F:\task\恒生股指期货\numba_突破max01_空策略pro01\csv_gather\%s.csv'%name
            ...
            html_path = r'F:\task\恒生股指期货\numba_突破max01_空策略pro01\策略介绍html\csv_html\%s.html' % name
            df_zong_html.to_html(html_path)

        # 单独生成html，读取本地数据
        if True == 0:
            df_zong = pd.read_csv(r'F:\task\恒生股指期货\numba_突破max01_空策略pro01\csv_gather\%s.csv'%name)
            print(df_zong.tail())
            # exit()
            df_zong_html = pd.DataFrame()
            for k, v in df_zong.groupby('s_time'):
