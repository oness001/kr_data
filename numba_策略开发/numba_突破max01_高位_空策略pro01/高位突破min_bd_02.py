...
def cal_signal_(df0,df1,df2,strat_time,end_time,cs1=0,cs2=0,cs3=0,cs4=0, sxf=1,slip=1):
    ...
    for i in range(10,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            max_bd_line = low_inner + max_bd
            open_line = max_log_high - xia_bd
            # 信号生成
            if we_trade_con:
                if now_pos == 0:
                    short_condition = (clsoe_bar < open_line) and (last_close > open_line) and (last_close>max_bd_line)
                    # 空：突破最低线。
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        df000 = df.copy()
        resdf,res0 = duojincheng_backtesting(df000, zong_can=[[50,130,250,50]], strat_time=strat_time, end_time=end_time, cpu_nums = 1)
        ...
        draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                        ...
                        },vol_bar=True, markline_show2=True,path='tpmax_02_%s'%timeer0)
        print(res0)
    ...
    #多进程策略 :
        ...
        canshu_list = []
        for cs1 in range(30, 70, 5):
            for cs2 in range(170, 230, 5):
                for cs3 in range(260, 330, 10):
                        for cs4 in range(50, 61, 5):
        ...
        print(resdf.tail())
        resdf.to_csv(r'numba_突破max01_策略02(精细)_%s.csv'%timeer0)
        print(f'总时间：{time.process_time() -time0}  s')