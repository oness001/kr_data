...
def cal_signal_(df0,df1,df2,strat_time,end_time,cs1=0,cs2=0,cs3=0,cs4=0, sxf=1,slip=1):
    ...
    for i in range(10,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            # 信号生成
            if we_trade_con:
                open_line = max_log_high-xia_bd
                if now_pos == 0:
                    short_condition = (clsoe_bar < open_line) and (last_close > open_line)
                    # 空：突破最低线。
...
def duojincheng_backtesting(df_input,zong_can,strat_time,end_time, cpu_nums=3):
    ...
    df2cols = ['candle_begin_time', 'open', 'high', 'low', 'close', 'volume',
              '止损价','止盈价','日内最低价','日内最高价','开仓线','atr_day']
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        df000 = df.copy()
        resdf,res0 = duojincheng_backtesting(df000, zong_can=[[60,200,60,50]],
        ...
        draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                        ...
                        },vol_bar=True, markline_show2=True,path='tp_max_05_%s'%timeer0)
        print(f'总时间：{time.process_time() - time0}  s')
    #多进程策略 :
    elif 1:
        canshu_list = []
        for cs1 in range(35, 75, 5):
            for cs2 in range(150, 260, 10):
                for cs3 in range(30, 81, 5):
        ...
        print(resdf.tail())
        print(f'总时间：{time.process_time() -time0}  s')
        resdf.to_csv(s_time[:7]+r'_numba_突破max01_策略05_%s.csv'%(timeer0))
