...
@jit(nopython=True)
def cal_signal_(df0,df1,df2,strat_time,end_time,cs1=0,cs2=0,cs3=0,cs4=0, sxf=1,slip=1):
    '''
    ...
    for i in range(10,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            stop_loss_price = now_open_prcie + stop_loss_n
            stop_win_price = now_open_prcie - stop_win_n

            # 信号生成
            if we_trade_con:
                base_line_con = (clsoe_bar < low_inner) and (last_close > low_inner) and (now_pos==0) and (not levea_base)
                if base_line_con:
                    levea_base = True
                    base_line = clsoe_bar
                    max_bd_line = clsoe_bar + max_bd
                if now_pos == 0:
                    short_condition = (clsoe_bar < max_bd_line) and (last_close > max_bd_line)
                    # 空：突破最低线。
                    if short_condition and inner_trade_nums == 1 :
                        inner_trade_nums = 0
                        df1[i][1] = -1

                elif now_pos < 0:
                    stop_loss_con = (clsoe_bar > stop_loss_price) and (last_close <= stop_loss_price)
                    stop_win_con = ((clsoe_bar < stop_win_price)) and (last_close >= stop_win_price)
                    close_chort = False
                    if close_chort :
                        df1[i][1] = 0
                    elif stop_loss_con:
            ...
            df2[i, 7] = stop_win_price
            df2[i, 8] = min_log_low
            df2[i, 9] = max_log_high
            df2[i, 10] = base_line
            df2[i, 11] = max_bd_line

    res0 = cal_tongji(df_input=df1)
    res0 = np.concatenate((res0,np.array([cs1,cs2,cs3,cs4])))
    return df0,df1,df2,res0
def cal_signal(df0,df1,df2,strat_time,end_time,cs1=0,cs2=0,cs3=0,cs4=0):
    a = time.process_time()
    df0, df1, df2, res = cal_signal_(df0, df1, df2, strat_time, end_time, cs1, cs2, cs3,cs4)
...
def duojincheng_backtesting(df_input,zong_can,strat_time,end_time, cpu_nums=3):
    ...
    df2cols = ['candle_begin_time', 'open', 'high', 'low', 'close', 'volume',
              '止损价','止盈价','日内最低价','日内最高价','条件线','开仓线','atr_day']
        ...
        for j in range(0, len(zong_can), cpu_nums):
            for i in range(cpu_nums):
                if j + i <= len(zong_can) - 1:
                    ...
                    cs1, cs2, cs3, cs4 = np.array(canshu0)
                    p.apply_async(cal_signal, args=(df0,df1,df2,strat_time,end_time,cs1, cs2, cs3, cs4,), callback=tianjia,)
                else:
                    break
        p.close()
        p.join()
    ...
    # 单进程测试
        ...
        for cs0 in zong_can:
            a = time.process_time()
            cs1, cs2, cs3, cs4 = np.array(cs0)
            df0, df1, df2, res0 = cal_signal(df0, df1, df2, strat_time, end_time, cs1, cs2, cs3, cs4)
            print('runingtime:', time.process_time() - a, 's')
            tianjia(res=res0)
        # 转换成df数据
        ...
        cols = ['最后收益', '最大收益', '最大回撤', '赢撤率', '夏普率', '平均收益', '开仓次数', '胜率', '盈亏比', '参数1', '参数2', '参数3', '参数4']
...
if __name__ == '__main__':
    ...
    # df_time_list = [['2020-1-01 09:15:00', '2020-12-26 16:25:00']]
    ...
    if 0:
        ...
        df000 = df.copy()
        resdf,res0 = duojincheng_backtesting(df000, zong_can=[[45,200,250,50]], strat_time=strat_time, end_time=end_time, cpu_nums = 1)
        ...
        draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                        ...
                        },vol_bar=True, markline_show2=True,path='tp_max_03_%s'%timeer0)
        print(f'总时间：{time.process_time() - time0}  s')
    #多进程策略 :
    elif 0:
        canshu_list = []
        for cs1 in range(35, 55, 5):
            for cs2 in range(150, 260, 10):
                for cs3 in range(200, 320, 10):
                        # for cs4 in range(30, 35, 5):
                        canshu_list.append([cs1, cs2, cs3,0])
        ...
        print(resdf.tail())
        resdf.to_csv(r'%s_numba_突破max01_策略03_%s.csv'%(s_time,timeer0))
        print(f'总时间：{time.process_time() -time0}  s')

    # 多进程策略 :
    elif 1:
        from 工具函数.多阶段回测 import duojieduan_huice
        ...
        print('参数列表个数：', len(canshu_list))
        time0 = time.process_time()
        func_canshu = {'zong_can':canshu_list[:10],'cpu_nums':2,"strat_time":strat_time,"end_time":end_time}
        df_zong =duojieduan_huice(df, duojincheng_backtesting, func_canshu, s_time, e_time, jiange = '4MS')
        df_zong.to_html('test.html')
