...
def cal_signal_(df0,df1,df2,strat_time,end_time,cs0, sxf=1,slip=1):
    ...
    '''
    # 配置参数
    stop_win_n,max_bd ,ma_len ,atrn,tradi_inner_nums= cs0
    # 配置临时变量
...
def cal_signal(df,strat_time,end_time,canshu):
    ...
    df2cols = ['candle_begin_time', '止损价', '止盈价', '日内最低价', '日内最高价', '开仓线',
               'atr_day', '小均线','n_atr']
    df['小均线'] = talib.SMA(df['close'], int(canshu[2]))
...
if __name__ == '__main__':
    ...
    if True == a:
        ...
        resdf,res0 = duojincheng_backtesting(df000, zong_can=[[130,270,20,3,2]], strat_time=strat_time, end_time=end_time, cpu_nums = 1)
        print(res0)
        print(resdf.tail(5))
        ...
        if mode == 1:
            ...
            draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                        ...
                        },vol_bar=True, markline_show2=True,path=r'html_gather\max_突破06_atr_%s'%timeer0)
        if mode == 2:
            draw_charts(resdf,canshu={'opne_price':resdf['opne_price'],
                                  '止损价' :resdf['止损价'],
                                  '止盈价':resdf['止盈价'],
    ...
    if True == b:
        [130, 270, 20, 3, 2]
        canshu_list = []
        for cs1 in range(100, 231, 10):
            for cs2 in range(150, 351, 10):
                for cs3 in range(10, 35, 4):
                    for cs4 in range(2, 6, 1):
                        for cs5 in range(1, 5, 1):
        ...
        print(resdf.tail())
        resdf.to_csv(r'csv_gather\max_突破06_atr_%s.csv'%timeer0)
        print(f'总时间：{time.process_time() -time0}  s')
    ...
    if True == c:
        ...
        if True == 1:
            [130, 270, 20, 3, 2]
            canshu_list = []
            for cs1 in range(100, 160, 10):
                for cs2 in range(200, 310, 10):
                    for cs3 in range(10, 21, 2):
                        for cs4 in range(2, 4, 1):
            ...
            path_csv = r'F:\task\恒生股指期货\numba_突破max01_空策略pro01\csv_gather\max_突破06_atr_2015_2019.csv'
            ...
            html_path = r'F:\task\恒生股指期货\numba_突破max01_空策略pro01\策略介绍html\csv_html\%s' % name
