...
# 功能函数
def transfer_to_period_data_pro(df, timecol,rule_type):
    """
    将数据转换为其他周期的数据

    """
    # ==转换为其他分钟数据
    period_df = df.resample(rule=rule_type, on=timecol, label='left', closed='left').agg(
        {'open': 'first',
         'high': 'max',
         'low': 'min',
         'close': 'last',
...
def transfer_period_anydata(df,cols={'close': 'last'}, rule_type="1T"):
    """
    将数据转换为其他周期的数据
    """
    # ==转换为其他分钟数据
    ...
    #      'close': 'last',
    #      'volume': 'sum',}
    period_df = df.resample(rule=rule_type, on='candle_begin_time', label='left', closed='left').agg(cols)
    period_df.reset_index(inplace=True)
    collist = list(cols.keys())
    collist.append('candle_begin_time')
    df = period_df[collist]
...
def get_local_hsi_csv(s_time, e_time,datapath):
    print('数据地址：', datapath)
    df = pd.read_csv(filepath_or_buffer=datapath)
    df['candle_begin_time'] = df['datetime']
    df['candle_begin_time'] = pd.to_datetime(df['candle_begin_time'])
...
def get_local_hsi_csv_pro(s_time, e_time,timecol="datetime",datapath=""):
    print('数据地址：', datapath)
    df = pd.read_csv(filepath_or_buffer=datapath)
    df[timecol] = pd.to_datetime(df[timecol])
    df = df[(df[timecol] >= pd.to_datetime(s_time)) & (df[timecol] <= pd.to_datetime(e_time))]
    df["candle_begin_time"] = df[timecol]
    df = df[["candle_begin_time", 'open', 'high', 'low', 'close', 'volume']]
    df.reset_index(inplace=True, drop=True)
    print('数据大小', df.shape)
    df.fillna(method='ffill', inplace=True)
