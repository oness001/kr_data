...
def cal_signal_(df0, df1, df2, strat_time, end_time, cs0):
    ...
    for i in range(2,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            if now_pos == 0:
                long_condition =  (ma_xiao > ma_zhong)and(ma_xiao2 <=  ma_zhong2)
                ...
                stop_loss_price = now_open_prcie-stop_n if (max_log_high-now_open_prcie < stop_n) else now_open_prcie + 50
...
def cal_signal(df,strat_time,end_time,cs0,ix_zong=100):
    a = time.process_time()
     # ===指标数据
    # da, zhong, xiao, stop_n, max_stop_win = cs0
    df['ma_da'] = talib.EMA(df['close'],cs0[0])
    df['ma_z'] = talib.EMA(df['close'],cs0[1])
    df['ma_xiao'] = talib.EMA(df['close'],cs0[2])
    N = int(cs0[0])
...
if __name__ == '__main__':
    ...
    if True == a:
        ...
        resdf,res0  = duojincheng_backtesting(df000, zong_can=[[240, 120, 20, 50.0,200]],
        ...
        if mode == 3:
            ...
            res_trades = []
            cols = ['开始时间','结束时间','持续分钟','盈亏大小','最大净值',"结束-最大",'最小净值',"结束-最小",]

            print(res0)

            for k ,v in resdf.groupby('pos_stime'):
                res0=[]
                res0.append(v.iloc[0]['candle_begin_time'])
                res0.append(v.iloc[-1]['candle_begin_time'])

                res0.append( (v.iloc[-1]['candle_begin_time']-v.iloc[0]['candle_begin_time'])/timedelta(minutes=1))
                res0.append(v.iloc[-1]['next_open_price'] - v.iloc[0]['opne_price'])
                res0.append(v['close'].max() - v.iloc[0]['opne_price'])
                res0.append(v['close'].max() - v.iloc[-1]['next_open_price'])
                res0.append(v.iloc[0]['opne_price']-v['close'].min())
                res0.append(v.iloc[-1]['next_open_price'] -v['close'].min())
                res_trades.append(res0)
            df = pd.DataFrame(res_trades,columns=cols)
            df=df.append( df.describe(),ignore_index=False)
    ...
    if True == b:
        ...
        s_time, e_time = df_time_list[0]
        df = get_local_hsi_csv(s_time, e_time, datapath)  # 获取本地数据
        for t0 in ["1T","3T","5T","15T","30T","45T","1H","2H","3H","4H"]:
            df0 = transfer_to_period_data(df, rule_type=t0)
            df000 = df0.copy()
            ...
            print(f'{t0}总时间: {time.process_time() - time0}  s')
