    ...
    for i in range(2,df0.shape[0]):#类似于vnpy:on_bar
        ...
        if trading_time_con :
            ...
            if now_pos == 0:
                ...
                long_condition &= (ma_xiao > ma_zhong)
...
def duojincheng_backtesting(df_input,zong_can,strat_time,end_time, cpu_nums=3):
    ...
    # 单进程测试
        ...
        print(huice_df)
        [end_zj, max_zj, max_back, yc_rate, sharp_rate, stn_rate, yk_risk_rate, mean_zj, counts, sl, yk_rate]
        cols = ['最后收益', '最大收益', '最大回撤', '赢撤率', '夏普率', '平均收益', '开仓次数', '胜率', '盈亏比'] + [f'参数{i}' for i in range(1, len(zong_can[-1]) + 1)]
        res0 = pd.DataFrame(huice_df)
        res0 = res0[cols]
...
if __name__ == '__main__':
    ...
    if True == a:
        ...
        resdf,res0  = duojincheng_backtesting(df000, zong_can=[[44, 140, 30, 50.0,140]],
