...
from numba_策略开发.功能工具.因子库 import rsi_cci_atr01 ,std_zdf_tp,rsi_cci_atr01_1
...
pd_display_rows = 10000
pd_display_cols = 100
pd_display_width = 100
...
pd.set_option('display.max_colwidth', pd_display_width*10)
        ...
        if trading_time_con :
            ...
            if we_trade_con:
                if now_pos == 0:
                    ...
                    max_log_high = max(high_bar, max_log_high)
                    if (max_log_high - now_open_prcie < 80):
                        stop_loss_price = stop_line
                    else:
                        stop_loss_price = max(now_open_prcie + 20,stop_line)
...
def cal_signal(df,strat_time,end_time,cs0,ix_zong=100,all_day=0):
    ...
    df['zuheyinzi'] = rsi_cci_atr01(df[['high', 'low', 'close']].values,n=int(cs0[0]))

    # df['zuheyinzi'] = df['zuheyinzi'] - df['zuheyinzi'].shift(1)
    # df['zuheyinzi'] = df['zuheyinzi'] - df['zuheyinzi'].shift(1)
    # df['zuheyinzi'] = talib.SMA(df['zuheyinzi'],int(cs0[0]))
    df['xiao'] =talib.SMA(df['close'],int(cs0[1]))
    df['stop_line'] =talib.MIN(df['close'],int(cs0[0]))
    df['止损价'] = np.nan
    df['止盈价'] = np.nan
    df['日内最低价'] = np.nan
    df['日内最高价'] = talib.MAX(df['close'],int(300))
    pass
    # ===转化成np.array
    ...
    print('runingtime:', time.process_time() - a, f's ,已经完成 ==:{round(ix_zong*100,2)}%')
...
if __name__ == '__main__':
    ...
    datapath = r'F:\vnpy_my_gitee\company\hsi_data_1min\HSI2011-2020-11.csv'
    ...
    if True == a:
        ...
        df_time_list = [['2018-1-1 09:15:00', '2018-6-30 16:25:00']]
        ...
        print(res0)
        # print(resdf[["candle_begin_time","signal",   "pos" , "opne_price",  "per_lr" , "sl"]].tail(1000))

        result_df = resdf[["candle_begin_time","signal",   "pos" , "opne_price",  "per_lr" , "sl"]]
        result_df['signal0'] = result_df['signal'].shift()
        result_df.loc[result_df['signal0']>= 0,'pos_strat'] = result_df.loc[result_df['signal0']>=0,'candle_begin_time']
        result_df["pos_strat"].fillna(method="ffill",inplace=True)

        result_df['sl'] = result_df['sl'].shift(-1)
        rs_pos_long = []

        for k,v in result_df.groupby('pos_strat'):
            # print(v)
            # exit()
            rs_pos_long.append([v.iloc[0]["candle_begin_time"],v.iloc[0]["pos"],len(v),v.iloc[-1]['sl']])
        rs_pos_long = pd.DataFrame(rs_pos_long, columns=["candle_begin_time", "pos", 'longtime', 'yl'])
        rs_pos_long = rs_pos_long[rs_pos_long["pos"] > 0]
        rs_pos_long.sort_values(by=['longtime'],ascending=True,inplace=True)
        print(rs_pos_long[rs_pos_long["pos"]>0].iloc[:-1])

        print(rs_pos_long.describe())
        # exit()
    ...
    if True == b:
        ...
        canshu_list = []
        for cs1 in range(25,66, 5):
            for cs2 in range(40, 71, 5):
                for cs3 in range(20, 51, 5):
                    for cs4 in range(1, 3, 1):
                        for cs5 in range(-10, 11, 5):
                            for cs6 in range(160, 221, 10):
        ...
        df_time_list = [['2016-1-1 09:15:00', '2019-12-30 16:25:00']]
        path=os.getcwd()+r'\csv_gather\%s_%s2.csv' % (filename, timeer0)
        random.shuffle(canshu_list)
        canshu_list = random.sample(canshu_list, int(0.15 * len(canshu_list)))
        random.shuffle(canshu_list)
        ...
        resdf = mp_backtesting(df000, cal_signal,zong_can=canshu_list[:],strat_time=strat_time, end_time=end_time,
                                        path=path,cpu_nums=3,all_day=ALL_DAY)

        try:
            df = pd.read_csv(path,index_col=0)
        except Exception:
            df = resdf[0].tail(10)
        df.sort_values(by="最后收益",inplace=True,ascending=True)
        df.reset_index(drop=True,inplace=True)

        print(df.tail(10),df.shape)
        print(f'总时间：{time.process_time() - time0}  s')
    ...
    if True == c:
        ...
        if True == 1:
            ...
            func_canshu = {'zong_can': canshu_list[:], 'cpu_nums': 2, "path" : r"","all_day":0,"strat_time": strat_time, "end_time": end_time}
            df_zong = duojieduan_huice(df, cal_signal, func_canshu, s_time, e_time, jiange='12MS')
