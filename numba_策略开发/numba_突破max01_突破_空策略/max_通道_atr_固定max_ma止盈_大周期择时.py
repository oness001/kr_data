    ...
    for i in range(10, df0.shape[0]):  # 类似于vnpy:on_bar
        ...
        if trading_time_con:
            ...
            if we_trade_con:
                ...
                if now_pos == 0:
                    short_condition =  (clsoe_bar < min_n_p) and (last_close >= min_n_p) and (clsoe_bar<da_ma)
                    # 空：突破最低线。
...
def cal_signal(df, strat_time, end_time, canshu):
    ...
    df2cols = ['candle_begin_time', '止损价', '止盈价', '日内最低价', '日内最高价', '开仓线',
               'atr_day', '小均线', 'n_atr','移动止赢','大均线','min_n']
    # 配置参数:止盈，ma_len（atr），atr_倍数，交易次数
...
if __name__ == '__main__':
    ...
    c = 1
    if True == a:
        df_time_list = [['2016-02-01 09:15:00', '2019-12-20 16:25:00']]
        ...
        # time.sleep(0.2)
        # stop_win_n, ma_len, da_ma_n, sw_atrn, trade_inner_nums = cs0
        # [205.0, 24.0, 2000.0, 3.0, 1.0]
        resdf, res0 = duojincheng_backtesting(df000, zong_can=[[205.0, 24.0, 2000.0, 3.0, 1.0]],
                                              strat_time=strat_time,
    ...
    if True == c:
        ...
        canshu_list = []
        for cs1 in range(200, 241, 5):
            for cs2 in range(20, 31, 2):
                for cs3 in [1800,2000,2300,3300,3500,3700]:
        ...
        strat_time = np.array([1, 9, 30])
        end_time = np.array([27, 16, 10])
        name = f'{filename}_{s_time[:4]}_{e_time[:4]}_5min_精细粒度'
