...
def cal_signal_(df0, df1, df2, strat_time, end_time, cs0, sxf=1, slip=1):
    ...
    stop_win_n, stop_loss_n, trade_inner_nums = cs0
    # 配置临时变量
    inner_trade_nums = trade_inner_nums
    open_pos_size = 1
    max_log_high = np.nan
    min_log_low = np.nan
    open_pos_high = np.nan
    for i in range(10, df0.shape[0]):  # 类似于vnpy:on_bar
        ...
        if trading_time_con:
            ...
            if we_trade_con:
                if now_pos == 0:
                    short_condition = (clsoe_bar < inner_low) and (last_close >= inner_low)
                    # 空：突破最低线。
                    if short_condition and inner_trade_nums > 0:
                        inner_trade_nums -= 1
                        df1[i][1] = -1
                        open_pos_high = max_log_high if max_log_high< (now_open_prcie + stop_loss_n) else (now_open_prcie + stop_loss_n)
                elif now_pos < 0:
...
if __name__ == '__main__':
    ...
    c = 1
    if True == a:
        df_time_list = [['2019-12-10 09:15:00', '2019-12-23 16:25:00']]
        ...
        # time.sleep(0.2)
        resdf, res0 = duojincheng_backtesting(df000, zong_can=[[220, 60,1]],
                                              strat_time=strat_time,
    ...
    if True == b:
        ...
        for cs1 in range(200, 280, 10):
            for cs2 in range(30, 70, 10):
                for cs3 in range(1, 2, 1):
                        canshu_list.append([cs1, cs2, cs3])
